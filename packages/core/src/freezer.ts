...
export class ElementFreezer {
  async freezeAll(
    ...
    if (freezeAnimations) {
      ...
      const destAnimated = frozenElement
        ...
        .map((a) => (a.effect as KeyframeEffect).target!);
      promises.push(
        ...sourceAnimated.map((src, i) =>
          Promise.resolve(this.freezeWebAnimation(src, destAnimated[i]))
        )
      );

      const sourceSmil = element.querySelectorAll('animate, animateTransform, animateMotion, set');
      const destSmil = frozenElement.querySelectorAll(
        'animate, animateTransform, animateMotion, set'
      );
      promises.push(
        ...Array.from(sourceSmil).map((src, i) =>
          this.freezeSmilAnimation(src as SVGAnimationElement, destSmil[i] as SVGAnimationElement)
        )
      );
    }
    ...
    if (freezeCanvas) {
      ...
      promises.push(
        ...Array.from(sourceCanvases).map((src, i) =>
          this.freezeCanvas(src as HTMLCanvasElement, destCanvases[i] as HTMLCanvasElement)
        )
      );
    }
    ...
    if (freezeVideo) {
      ...
      promises.push(
        ...Array.from(sourceVideos).map((src, i) =>
          this.freezeVideo(src as HTMLVideoElement, destVideos[i] as HTMLVideoElement)
        )
      );
    }
    ...
    if (freezeImages) {
      ...
      promises.push(
        ...Array.from(sourceImages).map((src, i) =>
          this.freezeImage(
            src as SVGImageElement | HTMLImageElement,
            destImages[i] as SVGImageElement | HTMLImageElement
          )
        )
      );
    }
  ...
  private async freezeSmilAnimation(
    source: SVGAnimationElement,
    dest: SVGAnimationElement
  ): Promise<void> {
  ...
  private async freezeImage(
    source: SVGImageElement | HTMLImageElement,
    dest: SVGImageElement | HTMLImageElement
  ): Promise<void> {
    if (!this.isVisible(dest)) {
